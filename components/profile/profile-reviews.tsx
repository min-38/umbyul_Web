    ...
    <div className="flex flex-col gap-4">
      ...
      <ul className="flex flex-col divide-y divide-zinc-200 dark:divide-zinc-800">
        {sorted.map((r) => {
          ...
          // 아이템(곡/앨범) 상세 링크. 행 전체를 링크로 감싸면 아티스트·앨범 링크를 못 넣어서
          // 커버·제목만 아이템으로, 아티스트/앨범은 각각 별도 링크로 분리(NON-24 개선).
          const href = r.spotifyId ? `/${r.targetType}/${r.spotifyId}` : null;
          const title = r.name ?? t("(알 수 없는 항목)");
          return (
            <li key={r.id}>
              <div className="flex items-center gap-3 py-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="flex items-center gap-1.5 text-sm font-medium text-zinc-900 dark:text-zinc-50">
                    {href ? (
                      <Link href={href} className="truncate hover:underline">{title}</Link>
                    ) : (
                  ...
                  <p className="truncate text-xs text-zinc-500">
                    {r.artists && r.artists.length > 0 ? <ArtistLinks artists={r.artists} /> : (r.artist ?? "")}
                    {r.targetType === "track" && r.albumId && (
                      <>
                        {" · "}
                        <Link href={`/album/${r.albumId}`} className="hover:underline">{r.albumName}</Link>
                      </>
                    )}
                  </p>
                </div>
