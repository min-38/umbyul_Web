// 경량 i18n. 한국어 문자열을 키로 쓰고, 로케일에 맞는 사전으로 치환.
// ko 로케일은 키(한국어)를 그대로 반환 → ko 사전 불필요. en/ja/es는 아래 사전 매핑.
export type Locale = "ko" | "en" | "ja" | "es";

// {name} 형태 파라미터 치환 + 복수형("단수형|복수형") + 로케일 소수점 표기 지원(QA10-1).
export function translate(locale: Locale, ko: string, params?: Record<string, string | number>): string {
  const dict = locale === "ko" ? null : DICTS[locale];
  let s = dict ? (dict[ko] ?? ko) : ko;
  if (params) {
    // 복수형: 사전 값이 "단수형|복수형"이면 count(없으면 score, 없으면 첫 숫자)로 선택.
    // ko/ja는 파이프를 쓰지 않으므로(조수사 체계) 그대로 통과.
    if (s.includes("|")) {
      const [one, many = one] = s.split("|");
      s = pluralCount(params) === 1 ? one : many;
    }
    for (const [k, v] of Object.entries(params)) {
      const val = typeof v === "number" ? formatNumber(v, locale) : String(v);
      s = s.replaceAll(`{${k}}`, val);
    }
  }
  return s;
}

// 복수형 선택 기준 수: count 우선, 없으면 score, 없으면 첫 숫자 파라미터.
function pluralCount(params: Record<string, string | number>): number | undefined {
  if (typeof params.count === "number") return params.count;
  if (typeof params.score === "number") return params.score;
  for (const v of Object.values(params)) if (typeof v === "number") return v;
  return undefined;
}

// 정수는 그대로 두어(연도·ID 그룹핑 방지) 소수만 로케일 소수점으로 — ES는 "3,5"(es), 그 외는 "3.5".
function formatNumber(v: number, locale: Locale): string {
  return Number.isInteger(v) ? String(v) : v.toLocaleString(locale);
}

// 한국어 → 영어 매핑. 문구를 i18n 대상으로 옮길 때 여기에 추가.
export const EN: Record<string, string> = {
  // ── 메타 ──
  "음악을 듣고 평가하고 기록하세요.": "Listen, rate, and remember music you love.",

  // ── 헤더 ──
  신규: "New",
  급상승: "Rising",
  차트: "Charts",
  발견: "Discover",
  추천: "Recommended",
  "당신이 좋아하는 장르들": "Genres you like",
  "오늘의 음악": "Today's Pick",
  "리뷰하기": "Write a review",
  "선호 장르": "Favorite genres",
  "내 선호 장르": "My genres",
  "추천에 반영돼요. 언제든 바꿀 수 있어요.": "Used for recommendations. Change anytime.",
  "저장됐어요": "Saved",
  "좋아하는 장르 (선택) — 추천에 반영돼요.": "Genres you like (optional) — used for recommendations.",
  "로그인 / 가입": "Log in / Sign up",
  홈: "Home",
  "앨범, 곡, 아티스트, 유저 검색": "Search albums, tracks, artists, users",

  // ── 유저 메뉴 ──
  프로필: "Profile",
  업적: "Achievements",
  설정: "Settings",
  로그아웃: "Log out",
  "로그아웃 하시겠어요?": "Log out?",

  // ── 푸터 ──
  정책: "Legal",
  고객지원: "Support",
  소식: "News",
  "서비스 소개": "About",
  후원하기: "Donate",
  "Glitter는 한 사람이 운영하는 서비스입니다. 서버비(호스팅·스토리지)에 커피 한 잔이 큰 힘이 됩니다.": "Glitter is run by one person. A cup of coffee toward server costs (hosting & storage) means a lot.",
  "순수 후원이라 어떤 대가나 혜택도 없어요. 부담 갖지 마시고, 마음이 있으실 때만.": "It's a pure donation — no perks or rewards. No pressure, only if you feel like it.",
  "Ko-fi에서 후원하기": "Support on Ko-fi",
  "후원 창구는 곧 열립니다.": "Donations open soon.",
  "결제·환불은 외부 후원 플랫폼에서 안전하게 처리됩니다.": "Payments and refunds are handled securely by an external platform.",
  공지사항: "Announcements",
  패치노트: "Patch notes",
  "페이지 이동": "Pagination",
  릴리스: "Releases",
  "작업 중": "In progress",
  "등록된 패치노트가 없습니다.": "No patch notes yet.",
  "등록된 공지가 없습니다.": "No announcements yet.",
  운영자: "Admin",
  "조회 {count}": "{count} view|{count} views",
  이전: "Prev",
  다음: "Next",
  "별점을 매길수록 별이 쌓여 반짝입니다 — 그래서 Glitter.":
    "The more you rate, the more the stars stack up and shine — that's Glitter.",
  이용약관: "Terms of Service",
  "개인정보 처리방침": "Privacy Policy",
  문의: "Contact",

  // ── 랜딩 ──
  "음악을 듣고, 평가하고, 기록하세요.": "Listen. Rate. Remember.",
  "좋아하는 앨범과 곡을 평가하고, 리뷰를 남기고, 취향이 맞는 사람을 팔로우하세요.":
    "Rate albums and tracks, write reviews, and follow people who share your taste.",
  둘러보기: "Explore",
  시작하기: "Get started",
  "0.5점 단위 별점": "Half-star ratings",
  "곡·앨범을 세밀하게 평가하고 나만의 기록을 남기세요.":
    "Rate tracks and albums precisely and keep your own log.",
  "취향 팔로우": "Follow tastes",
  "취향이 맞는 유저를 팔로우하고 새 평가를 받아보세요.":
    "Follow people who share your taste and catch their new ratings.",
  "곡을 골라 '이거 들어봐' 믹스를 만들면, 유튜브·스포티파이로 듣고 평가해요.":
    "Build a 'listen to this' mix — others can play it on YouTube or Spotify and rate the tracks.",
  "리뷰 & 댓글": "Reviews & comments",
  "리뷰를 쓰고 댓글로 이야기 나누세요. 좋은 리뷰엔 좋아요.": "Write reviews and chat about it in the comments. Like the good ones.",
  "발견 & 차트": "Discover & charts",
  "신규·급상승 음악을 발견하고, 랭킹으로 흐름을 봐요.": "Find new and rising music, and follow the rankings.",
  "곡·앨범 평점이 시간에 따라 어떻게 움직이는지 그래프로 봐요.": "See how a track or album's rating moves over time on a graph.",

  // ── 설정 탭 ──
  계정: "Account",
  알림: "Notifications",
  화면: "Display",
  연동: "Integrations",
  언어: "Language",
  테마: "Theme",
  라이트: "Light",
  다크: "Dark",
  시스템: "System",
  "화면 밝기 테마를 선택합니다. 시스템은 기기 설정을 따릅니다.":
    "Choose the display theme. System follows your device setting.",
  "표시 언어입니다.": "The language shown across the app.",

  // ── 프로필 ──
  팔로워: "Followers",
  팔로잉: "Following",
  팔로우: "Follow",
  "프로필 편집": "Edit profile",
  "작성한 리뷰": "Reviews",
  "받은 좋아요 {count}": "{count} like received|{count} likes received",
  "리뷰 {count}": "{count} review|{count} reviews",
  "다음 레벨까지 {xp} XP": "{xp} XP to next level",
  "아직 작성한 리뷰가 없습니다.": "No reviews yet.",
  "(알 수 없는 항목)": "(Unknown item)",
  "관리자에 의해 삭제되었습니다.": "Removed by an admin.",
  "계정이 영구 정지되어 작성이 제한됩니다.": "Your account is permanently suspended; posting is disabled.",
  "{until}까지 이용이 정지되어 작성이 제한됩니다.": "You are suspended until {until}; posting is disabled.",
  "사유": "Reason",
  "경고를 받았습니다.": "You have received a warning.",
  "새 공지사항이 등록되었어요.": "A new announcement was posted.",
  "일시 정지되어 평가할 수 없습니다.": "You're temporarily suspended and can't rate.",
  "계정이 정지되어 평가할 수 없습니다.": "Your account is suspended, so you can't rate.",
  "시행일": "Effective date",
  "문서를 준비 중입니다.": "This document is being prepared.",
  "자주 묻는 질문": "Frequently Asked Questions",
  검색: "Search",
  "최근 검색": "Recent searches",
  "검색 결과가 없습니다.": "No results found.",
  "등록된 질문이 없습니다.": "No questions yet.",
  "들은 음악을, 나만의 기록으로.": "Turn what you hear into your own record.",
  "Glitter는 음악 감상을 기록하고 나누는 서비스입니다. 방대한 음악 카탈로그 위에서, 당신의 별점과 리뷰가 쌓여 취향의 지도가 됩니다.":
    "Glitter is a place to log and share what you listen to. On top of a vast music catalog, your ratings and reviews build a map of your taste.",
  "무엇을 할 수 있나요": "What you can do",
  "Glitter에서 평가하고 리뷰하세요.": "Rate and review on Glitter.",
  "페이지를 찾을 수 없습니다.": "Page not found.",
  "홈으로": "Go home",
  "다시 시도": "Try again",
  "일시적인 오류가 발생했습니다.": "Something went wrong.",
  주: "Week",
  최근: "Recent",
  "내 최근 리뷰": "My recent reviews",
  전체: "All",
  "최다 리뷰": "Most reviews",
  "최고 평가": "Top rated",
  대상: "Type",
  정렬: "Sort",
  좋아요: "Likes",
  성별: "Gender",
  나이대: "Age",
  "10대": "Teens",
  "20대": "20s",
  "30대": "30s",
  "40대": "40s",
  "50대+": "50+",
  "아직 차트에 오른 항목이 없습니다.": "Nothing on the chart yet.",
  "음악 차트": "Music chart",
  "유저 차트": "User chart",
  "곡·앨범·아티스트 랭킹": "Tracks · Albums · Artists",
  "리뷰어 랭킹": "Top reviewers",
  화제순: "Hot",
  "좋아요 많은 순": "Most liked",
  "좋아요 비율 높은 순": "Highest like ratio",
  보기: "View",
  카드형: "Card",
  축약형: "Compact",
  "정렬 기준": "Sort by",
  "피드가 비어 있습니다.": "Your feed is empty.",
  피드: "Feed",
  "이름의 의미": "Behind the name",
  "지금 첫 별점을 남겨보세요.": "Leave your first rating.",
  "문의하기": "Contact us",
  카테고리: "Category",
  제목: "Title",
  내용: "Message",
  "평가·리뷰": "Ratings & Reviews",
  버그: "Bug",
  보내기: "Send",
  "보내는 중…": "Sending…",
  "문의가 접수되었습니다.": "Your message has been sent.",
  "답변은 입력하신 이메일로 보내드립니다.": "We'll reply to the email you provided.",
  "이메일 형식을 확인하세요.": "Please enter a valid email address.",
  "제목을 입력하세요.": "Please enter a title.",
  "내용을 입력하세요.": "Please enter your message.",
  "불러오는 중…": "Loading…",
  "모두 확인했어요": "You're all caught up",
  "본문으로 건너뛰기": "Skip to content",
  "확인을 위해 사용자명을 입력하세요": "Enter your username to confirm",
  "기본 정보": "Basic info",
  "생년월일을 확인하세요.": "Please check your date of birth.",
  "국가": "Country",
  "국가·성별": "Country & gender",
  "이 정보는 다음 날짜 이후 다시 변경할 수 있습니다.": "You can change this again after",
  "약관이 업데이트되었습니다": "Our terms have been updated",
  "계속하려면 변경된 내용을 확인하고 동의해주세요.": "Please review the changes and agree to continue.",
  "위 내용을 확인했으며 동의합니다": "I have read and agree to the above",
  "동의하고 계속": "Agree and continue",
  "동의 처리 중 문제가 발생했습니다. 다시 시도해주세요.": "Something went wrong while saving your consent. Please try again.",
  "비밀번호 표시": "Show password",
  "비밀번호 숨기기": "Hide password",
  "입력하신 이메일·문의 내용은 답변 목적으로만 사용되며, 처리 후 파기됩니다.": "Your email and message are used only to reply and are deleted afterward.",
  "문서 언어": "Document language",
  "메뉴": "Menu",
  "탐색": "Navigation",
  "이 사용자를 차단할까요?": "Block this user?",
  "알림을 모두 지울까요?": "Clear all notifications?",
  "만들지 못했어요. 잠시 후 다시 시도해주세요.": "Couldn't create it. Please try again shortly.",
  "아직 없습니다.": "None yet.",
  평점순: "Top rated",
  좋아요순: "Most liked",

  // ── 상세 (앨범/곡) ──
  곡: "Track",
  앨범: "Album",
  리뷰: "Reviews",
  "{count}개 평가": "{count} rating|{count} ratings",
  "Spotify에서 듣기": "Listen on Spotify",
  "YouTube에서 보기": "Watch on YouTube",
  트랙리스트: "Tracklist",
  정보: "Info",
  "{count}곡": "{count} track|{count} tracks",
  발매일: "Release date",
  "트랙 수": "Tracks",
  저작권: "Copyright",
  "평점 추이": "Rating trend",
  추이: "Trend",
  "(수정됨)": "(edited)",
  "이 댓글을 삭제할까요?": "Delete this comment?",
  "평가가 더 쌓이면 추이가 표시됩니다.": "The trend appears once more ratings accumulate.",
  "MusicBrainz에서 보기": "View on MusicBrainz",
  장르: "Genre",
  "아직 이 장르의 리뷰가 없어요.": "No reviews in this genre yet.",
  "아직 장르 태그가 없습니다.": "No genre tags yet.",
  길이: "Length",
  "아직 리뷰가 없습니다. 첫 평가를 남겨보세요.": "No reviews yet. Be the first to rate.",
  인기순: "Popular",
  최신순: "Newest",
  평가하기: "Rate",
  "내 평가 수정": "Edit my rating",

  // ── 검색 ──
  "검색어를 입력하세요.": "Enter a search term.",
  "검색 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.":
    "Something went wrong. Please try again shortly.",
  '"{q}"에 대한 검색 결과가 없습니다.': 'No results for "{q}".',
  "이 카테고리엔 결과가 없습니다.": "No results in this category.",
  "더 보기": "Show more",
  "검색 중…": "Searching…",
  아티스트: "Artist",
  유저: "User",

  // ── 평가 모달 ──
  "별점을 선택해주세요.": "Please select a rating.",
  "리뷰를 남겨주세요 (최소 10자)": "Write a review (min 10 characters)",
  "리뷰는 최소 10자 이상 작성해주세요.": "Reviews must be at least 10 characters.",
  "최소 10자 이상 작성해주세요.": "Write at least 10 characters.",
  "리뷰를 작성하면 경험치 10 XP를 받아요.": "You earn 10 XP for writing a review.",
  "평가를 삭제하면 경험치 10 XP가 차감돼요. 삭제할까요?": "Deleting this rating removes the 10 XP you earned. Delete it?",
  "욕설·비방은 금지됩니다. 비판은 좋지만 비난은 안 됩니다.":
    "No profanity or slander. Criticism is welcome, personal attacks are not.",
  "부적절한 리뷰는 신고 없이도 관리자가 삭제하고 제재할 수 있습니다.":
    "Admins may remove inappropriate reviews and take action against the author, even without a report.",
  삭제: "Delete",
  취소: "Cancel",
  "저장 중…": "Saving…",
  수정: "Update",
  수정하기: "Edit", // 메뉴 액션(편집 진입). 제출 버튼 '수정'(Update)과 구분(QA10-1)
  등록: "Submit",

  // ── 신고 ──
  "음악과 무관한 내용": "Not about music",
  "악플·욕설": "Abuse or insults",
  "부적절한 이름·프로필 사진": "Inappropriate name or profile picture",
  기타: "Other",
  "사유를 선택해주세요.": "Please select a reason.",
  신고: "Report",
  "신고가 접수되었습니다.": "Your report has been submitted.",
  닫기: "Close",
  "리뷰 신고": "Report review",
  "유저 신고": "Report user",
  "신고 내용은 운영자가 검토합니다.": "Reports are reviewed by our admins.",
  "상세 내용 (선택)": "Details (optional)",
  "접수 중…": "Submitting…",
  "관심 없음": "Not interested",
  "피드에서 숨겼어요": "Hidden from your feed",
  "실행 취소": "Undo",
  더보기: "More",
  차단: "Block",
  "차단 해제": "Unblock",
  "차단한 사용자입니다. 리뷰를 보려면 차단을 해제하세요.": "You've blocked this user. Unblock to see their reviews.",
  "차단 관리": "Blocked users",
  "차단한 유저는 서로의 리뷰·프로필이 보이지 않습니다.": "You and blocked users can't see each other's reviews or profiles.",
  "차단한 유저가 없습니다.": "You haven't blocked anyone.",

  // ── 알림 ──
  "님이 회원님을 팔로우했습니다": " followed you",
  "님이 회원님의 리뷰를 좋아합니다": " liked your review",
  "님이 댓글에서 회원님을 언급했습니다": " mentioned you in a comment",
  "알림이 없습니다.": "No notifications.",
  "알림 설정": "Notification settings",
  "모두 지우기": "Clear all",
  "알림 삭제": "Delete notification",

  // ── 로그인 ──
  환영합니다: "Welcome",
  이메일: "Email",
  비밀번호: "Password",
  로그인: "Log in",
  "비밀번호를 잊으셨나요?": "Forgot your password?",
  또는: "or",
  "처음이신가요?": "New here?",
  회원가입: "Sign up",
  로그인됨: "Logged in",

  // ── OAuth ──
  "Google로 계속하기": "Continue with Google",
  "Discord로 계속하기": "Continue with Discord",

  // ── 회원가입 ──
  "6자리 코드를 입력하세요.": "Enter the 6-digit code.",
  "코드가 올바르지 않거나 만료되었습니다.": "The code is incorrect or has expired.",
  "인증 메일을 보냈습니다": "We sent a verification email",
  "받은 편지함에서 6자리 코드를 입력하세요.": "Enter the 6-digit code from your inbox.",
  "6자리 코드": "6-digit code",
  확인: "Confirm",
  "메일이 안 보이면 스팸함을 확인하세요.": "If you don't see it, check your spam folder.",
  "가입을 환영합니다": "Create your account",
  "확인 중…": "Checking…",
  "사용 가능": "Available",
  "올바른 이메일 형식이 아닙니다.": "Please enter a valid email address.",
  "이미 가입된 이메일입니다.": "This email is already registered.",
  "8자 이상": "At least 8 characters",
  "대소문자 포함": "Upper and lowercase letters",
  "숫자 포함": "A number",
  "특수문자 포함": "A special character",
  "비밀번호 확인": "Confirm password",
  "비밀번호가 일치하지 않습니다.": "Passwords do not match.",
  "{link}에 동의합니다.": "I agree to the {link}.",
  가입하기: "Sign up",
  "이미 계정이 있으신가요?": "Already have an account?",

  // ── 온보딩 ──
  "거의 다 됐어요": "Almost there",
  "프로필을 완성해주세요.": "Complete your profile.",
  "영문·숫자·하이픈, 2–30자.": "Letters, numbers, hyphens. 2–30 chars.",
  "이미 사용 중": "Already taken",
  "사용할 수 없는 형식": "Invalid format",
  년: "Year",
  월: "Month",
  일: "Day",
  "만 14세 이상만 가입할 수 있습니다.": "You must be at least 14 years old.",
  "올바른 날짜를 선택하세요.": "Please select a valid date.",
  생년월일: "Date of birth",
  "성별 (선택)": "Gender (optional)",
  남성: "Male",
  여성: "Female",
  비공개: "Prefer not to say",
  완료: "Done",

  // ── 비밀번호 찾기/재설정 ──
  "재설정 링크를 보냈습니다": "Reset link sent",
  "{email}의 메일함을 확인해주세요.": "Check the inbox for {email}.",
  로그인으로: "Back to login",
  "비밀번호 재설정": "Reset password",
  "가입한 이메일로 재설정 링크를 보내드립니다.": "We'll send a reset link to your email.",
  "재설정 링크 보내기": "Send reset link",
  "로그인으로 돌아가기": "Back to login",
  "비밀번호는 8자 이상이어야 합니다.": "Password must be at least 8 characters.",
  "비밀번호가 변경되었습니다": "Password changed",
  "새 비밀번호로 다시 로그인해주세요.": "Please sign in again with your new password.",
  로그인하기: "Log in",
  "유효하지 않은 접근입니다": "Invalid link",
  "재설정 링크가 만료되었거나 잘못되었습니다.": "The reset link has expired or is invalid.",
  "재설정 링크 다시 받기": "Get a new reset link",
  "새 비밀번호 설정": "Set a new password",
  "새 비밀번호 (8자 이상)": "New password (8+ characters)",
  "새 비밀번호 확인": "Confirm new password",
  "비밀번호 변경": "Change password",

  // ── 설정: 계정 ──
  "변경되었습니다.": "Updated.",
  "username 형식(2~30자, 영문/숫자/하이픈)을 확인하세요.":
    "Check the username format (2–30 chars, letters/numbers/hyphens).",
  "설정되었습니다.": "Done.",
  "계정 정보": "Account info",
  가입일: "Joined",
  "연동 계정": "Linked accounts",
  아바타: "Avatar",
  "업로드 중…": "Uploading…",
  "이미지 변경": "Change image",
  "jpg, png, webp · 최대 5MB": "jpg, png, webp · up to 5MB",
  닉네임: "Display name",
  변경: "Save",
  "비밀번호 설정": "Set password",
  "소셜 로그인 계정입니다. 비밀번호를 설정하면 이메일로도 로그인할 수 있습니다.":
    "This is a social login account. Set a password to also sign in with email.",
  "새 비밀번호": "New password",
  "회원 탈퇴": "Delete account",
  "계정과 모든 데이터가 영구 삭제됩니다.": "Your account and all data will be permanently deleted.",
  "처리 중…": "Processing…",
  // 이메일 변경 · 데이터 내보내기 (NON-111)
  "이메일 변경": "Change email",
  "새 이메일로 확인 메일을 보냈습니다. 링크를 눌러야 변경이 완료됩니다.":
    "A confirmation email was sent to the new address. Click the link to finish the change.",
  "변경하려면 새 이메일로 온 확인 링크를 눌러야 합니다.":
    "To change it, click the confirmation link sent to your new email.",
  "공개 설정": "Privacy",
  "리뷰어 레벨 숨기기": "Hide reviewer level",
  "켜면 피드·리뷰·차트·프로필에서 내 레벨 뱃지가 표시되지 않습니다.": "When on, your level badge is hidden across feed, reviews, charts, and profile.",
  "데이터 내보내기": "Export data",
  "내 프로필·평가·팔로우·댓글을 JSON 파일로 내려받습니다.":
    "Download your profile, ratings, follows, and comments as a JSON file.",
  "JSON 내보내기": "Export JSON",
  "내보내는 중…": "Exporting…",
  "내보내기 파일을 저장했습니다.": "Your export file has been saved.",

  // ── 설정: 알림 ──
  "전체 알림": "All notifications",
  "모든 알림을 받습니다. 끄면 아래 항목과 무관하게 알림이 오지 않습니다.":
    "Receive all notifications. When off, nothing is sent regardless of the items below.",
  "팔로우 알림": "Follow notifications",
  "다른 유저가 회원님을 팔로우할 때": "When another user follows you",
  "리뷰 좋아요 알림": "Review like notifications",
  "회원님의 리뷰에 좋아요가 달릴 때": "When someone likes your review",

  // ── 설정: 연동 ──
  "준비 중입니다.": "Coming soon.",

  // ── 별점 입력(접근성) ──
  "{score}점": "{score} star|{score} stars",

  // ── 리뷰 댓글 & 공유 (NON-36) ──
  댓글: "Comments",
  "댓글 {count}": "{count} comment|{count} comments",
  "첫 댓글을 남겨보세요.": "Be the first to comment.",
  "댓글 달기…": "Add a comment…",
  "로그인하고 댓글 달기": "Log in to comment",
  답글: "Reply",
  "답글 {count}개 보기": "View {count} reply|View {count} replies",
  간략히: "Show less",
  "평가 없음": "No rating",
  "삭제된 댓글입니다.": "This comment was deleted.",
  "댓글 신고": "Report comment",
  "멘션 알림": "Mention notifications",
  "다른 유저가 댓글에서 회원님을 @태그할 때": "When someone @mentions you in a comment",
  "멘션 알림 끄기": "Mute mentions",
  "멘션 알림 꺼짐": "Mentions muted",
  "이 페이지에서 멘션 알림 끄기": "Mute mention notifications on this page",
  "이 페이지의 멘션 알림이 꺼져 있습니다": "Mention notifications are off for this page",
  믹스: "Mix",
  "믹스 만들기": "Create a mix",
  "믹스 제목": "Mix title",
  "한마디 (선택) — 이거 들어봐": "A note (optional) — listen to this",
  "듣기 링크 (선택) — 유튜브·스포티파이 등": "Listen link (optional) — YouTube, Spotify, etc.",
  만들기: "Create",
  "만드는 중…": "Creating…",
  듣기: "Listen",
  "이 믹스를 삭제할까요?": "Delete this mix?",
  "아직 담긴 곡이 없습니다.": "No tracks yet.",
  "곡 검색해서 담기…": "Search tracks to add…",
  "아직 믹스가 없습니다.": "No mixes yet.",
  담기: "Add",
  "최대 {count}곡까지 담을 수 있어요.": "Up to {count} tracks.",
  "유튜브 링크": "YouTube link",
  "유튜브에서 보기": "Watch on YouTube",
  "http(s) 링크만 넣을 수 있어요.": "Only http(s) links are allowed.",
  설명: "Description",
  "플레이리스트 링크": "Playlist link",
  "위로 이동": "Move up",
  저장: "Save",
  "곡 수정": "Edit track",
  "노래 변경": "Change track",
  "믹스 검색": "Search mixes",
  "믹스 수정": "Edit mix",
  목록으로: "Back to list",
  발견으로: "Back to Discover",
  "믹스 신고": "Report mix",
  안내: "Help",
  저장됨: "Saved",
  "순서는 화살표로 바꿀 수 있어요.": "Reorder with the arrows.",
  "곡을 검색해 담고, 유튜브 링크를 붙이면 스포티파이 없이도 들을 수 있어요.": "Search and add tracks; add a YouTube link so people without Spotify can listen too.",
  "아래로 이동": "Move down",
  공유: "Share",
  복사됨: "Copied",
  "{username}님의 리뷰": "{username}'s review",

  // ── 아티스트 상세 (NON-13) ──
  "Spotify에서 열기": "Open on Spotify",
  디스코그래피: "Discography",
  싱글: "Single",
  컴필레이션: "Compilation",
  "평가 좋은 트랙": "Top-rated tracks",
  "평가 좋은 앨범": "Top-rated albums",
  "아직 평가된 곡이 없습니다.": "No rated tracks yet.",
  "아직 평가된 앨범이 없습니다.": "No rated albums yet.",
  "평가된 릴리스 {rated} · 총 평가 {total}": "{rated} releases rated · {total} ratings",
  "커뮤니티 최근 리뷰": "Recent community reviews",
  "음악 정보를 불러오지 못했습니다. 잠시 후 다시 시도해주세요.":
    "Couldn't load music info. Please try again shortly.",
  "화제의 릴리스": "Trending releases",
};

// 한국어 → 일본어.
const JA: Record<string, string> = {
  "음악을 듣고 평가하고 기록하세요.": "音楽を聴いて、評価して、記録しよう。",

  신규: "新着",
  급상승: "急上昇",
  차트: "チャート",
  발견: "発見",
  추천: "おすすめ",
  "당신이 좋아하는 장르들": "好きなジャンル",
  "오늘의 음악": "今日のピックアップ",
  "리뷰하기": "レビューする",
  "선호 장르": "好みのジャンル",
  "내 선호 장르": "マイジャンル",
  "추천에 반영돼요. 언제든 바꿀 수 있어요.": "おすすめに反映されます。いつでも変更できます。",
  "저장됐어요": "保存しました",
  "좋아하는 장르 (선택) — 추천에 반영돼요.": "好きなジャンル（任意）— おすすめに反映されます。",
  "로그인 / 가입": "ログイン / 登録",
  홈: "ホーム",
  "앨범, 곡, 아티스트, 유저 검색": "アルバム・曲・アーティスト・ユーザーを検索",

  프로필: "プロフィール",
  업적: "実績",
  설정: "設定",
  로그아웃: "ログアウト",
  "로그아웃 하시겠어요?": "ログアウトしますか？",

  정책: "ポリシー",
  고객지원: "サポート",
  소식: "ニュース",
  "서비스 소개": "サービス紹介",
  후원하기: "支援する",
  "Glitter는 한 사람이 운영하는 서비스입니다. 서버비(호스팅·스토리지)에 커피 한 잔이 큰 힘이 됩니다.": "Glitterは一人で運営しています。サーバー費（ホスティング・ストレージ）へのコーヒー一杯が大きな支えになります。",
  "순수 후원이라 어떤 대가나 혜택도 없어요. 부담 갖지 마시고, 마음이 있으실 때만.": "純粋な支援で、見返りや特典はありません。無理せず、お気持ちのある時だけどうぞ。",
  "Ko-fi에서 후원하기": "Ko-fiで支援する",
  "후원 창구는 곧 열립니다.": "支援の受付は近日公開します。",
  "결제·환불은 외부 후원 플랫폼에서 안전하게 처리됩니다.": "決済・返金は外部の支援プラットフォームが安全に処理します。",
  공지사항: "お知らせ",
  패치노트: "パッチノート",
  "페이지 이동": "ページ移動",
  릴리스: "リリース",
  "작업 중": "作業中",
  "등록된 패치노트가 없습니다.": "パッチノートはまだありません。",
  "등록된 공지가 없습니다.": "お知らせはまだありません。",
  운영자: "運営",
  "조회 {count}": "閲覧 {count}",
  이전: "前へ",
  다음: "次へ",
  "별점을 매길수록 별이 쌓여 반짝입니다 — 그래서 Glitter.":
    "評価するほど星が積み重なって輝く — だから Glitter。",
  이용약관: "利用規約",
  "개인정보 처리방침": "プライバシーポリシー",
  문의: "お問い合わせ",

  "음악을 듣고, 평가하고, 기록하세요.": "聴いて。評価して。記録する。",
  "좋아하는 앨범과 곡을 평가하고, 리뷰를 남기고, 취향이 맞는 사람을 팔로우하세요.":
    "好きなアルバムや曲を評価し、レビューを書き、趣味の合う人をフォローしよう。",
  둘러보기: "見てまわる",
  시작하기: "はじめる",
  "0.5점 단위 별점": "0.5刻みの星評価",
  "곡·앨범을 세밀하게 평가하고 나만의 기록을 남기세요.":
    "曲やアルバムを細かく評価して、自分だけの記録を残そう。",
  "취향 팔로우": "趣味をフォロー",
  "취향이 맞는 유저를 팔로우하고 새 평가를 받아보세요.":
    "趣味の合うユーザーをフォローして、新しい評価を受け取ろう。",
  "곡을 골라 '이거 들어봐' 믹스를 만들면, 유튜브·스포티파이로 듣고 평가해요.":
    "曲を選んで「これ聴いて」ミックスを作れば、YouTubeやSpotifyで聴いて評価できます。",
  "리뷰 & 댓글": "レビューとコメント",
  "리뷰를 쓰고 댓글로 이야기 나누세요. 좋은 리뷰엔 좋아요.": "レビューを書いてコメントで語り合い、良いレビューにいいねを。",
  "발견 & 차트": "発見とチャート",
  "신규·급상승 음악을 발견하고, 랭킹으로 흐름을 봐요.": "新着・急上昇の音楽を見つけ、ランキングで流れをつかみます。",
  "곡·앨범 평점이 시간에 따라 어떻게 움직이는지 그래프로 봐요.": "曲やアルバムの評価が時間とともにどう動くかをグラフで見られます。",

  계정: "アカウント",
  알림: "通知",
  화면: "表示",
  연동: "連携",
  언어: "言語",
  테마: "テーマ",
  라이트: "ライト",
  다크: "ダーク",
  시스템: "システム",
  "화면 밝기 테마를 선택합니다. 시스템은 기기 설정을 따릅니다.":
    "表示テーマを選びます。システムは端末の設定に従います。",
  "표시 언어입니다.": "アプリ全体で表示される言語です。",

  팔로워: "フォロワー",
  팔로잉: "フォロー中",
  팔로우: "フォロー",
  "프로필 편집": "プロフィール編集",
  "작성한 리뷰": "レビュー",
  "받은 좋아요 {count}": "いいね {count} 件",
  "리뷰 {count}": "レビュー {count} 件",
  "다음 레벨까지 {xp} XP": "次のレベルまで {xp} XP",
  "아직 작성한 리뷰가 없습니다.": "まだレビューがありません。",
  "(알 수 없는 항목)": "（不明な項目）",
  "관리자에 의해 삭제되었습니다.": "管理者によって削除されました。",
  "계정이 영구 정지되어 작성이 제한됩니다.": "アカウントが永久停止されており、投稿できません。",
  "{until}까지 이용이 정지되어 작성이 제한됩니다.": "{until} まで利用停止中のため投稿できません。",
  "사유": "理由",
  "경고를 받았습니다.": "警告を受けました。",
  "새 공지사항이 등록되었어요.": "新しいお知らせが投稿されました。",
  "일시 정지되어 평가할 수 없습니다.": "一時停止中のため評価できません。",
  "계정이 정지되어 평가할 수 없습니다.": "アカウントが停止中のため評価できません。",
  "시행일": "施行日",
  "문서를 준비 중입니다.": "ドキュメントを準備中です。",
  "자주 묻는 질문": "よくある質問",
  검색: "検索",
  "최근 검색": "最近の検索",
  "검색 결과가 없습니다.": "検索結果がありません。",
  "등록된 질문이 없습니다.": "質問がまだありません。",
  "들은 음악을, 나만의 기록으로.": "聴いた音楽を、自分だけの記録に。",
  "Glitter는 음악 감상을 기록하고 나누는 서비스입니다. 방대한 음악 카탈로그 위에서, 당신의 별점과 리뷰가 쌓여 취향의 지도가 됩니다.":
    "Glitter は音楽鑑賞を記録して共有するサービスです。膨大な音楽カタログの上に、あなたの評価とレビューが積み重なって趣味の地図になります。",
  "무엇을 할 수 있나요": "できること",
  "Glitter에서 평가하고 리뷰하세요.": "Glitter で評価してレビューしよう。",
  "페이지를 찾을 수 없습니다.": "ページが見つかりません。",
  "홈으로": "ホームへ",
  "다시 시도": "再試行",
  "일시적인 오류가 발생했습니다.": "問題が発生しました。",
  주: "週",
  최근: "最近",
  "내 최근 리뷰": "最近の自分のレビュー",
  전체: "すべて",
  "최다 리뷰": "レビュー数順",
  "최고 평가": "高評価順",
  대상: "種類",
  정렬: "並び替え",
  좋아요: "いいね",
  성별: "性別",
  나이대: "年代",
  "10대": "10代",
  "20대": "20代",
  "30대": "30代",
  "40대": "40代",
  "50대+": "50代+",
  "아직 차트에 오른 항목이 없습니다.": "まだチャートに項目がありません。",
  "음악 차트": "音楽チャート",
  "유저 차트": "ユーザーチャート",
  "곡·앨범·아티스트 랭킹": "曲・アルバム・アーティスト",
  "리뷰어 랭킹": "レビュアーランキング",
  화제순: "話題",
  "좋아요 많은 순": "いいねが多い順",
  "좋아요 비율 높은 순": "いいね率が高い順",
  보기: "表示",
  카드형: "カード",
  축약형: "コンパクト",
  "정렬 기준": "並び替え基準",
  "피드가 비어 있습니다.": "フィードが空です。",
  피드: "フィード",
  "이름의 의미": "名前の由来",
  "지금 첫 별점을 남겨보세요.": "最初の評価を残そう。",
  "문의하기": "お問い合わせ",
  카테고리: "カテゴリ",
  제목: "件名",
  내용: "内容",
  "평가·리뷰": "評価・レビュー",
  버그: "バグ",
  보내기: "送信",
  "보내는 중…": "送信中…",
  "문의가 접수되었습니다.": "お問い合わせを受け付けました。",
  "답변은 입력하신 이메일로 보내드립니다.": "ご入力いただいたメールに返信します。",
  "이메일 형식을 확인하세요.": "メールアドレスの形式を確認してください。",
  "제목을 입력하세요.": "件名を入力してください。",
  "내용을 입력하세요.": "内容を入力してください。",
  "불러오는 중…": "読み込み中…",
  "모두 확인했어요": "すべて表示しました",
  "본문으로 건너뛰기": "本文へスキップ",
  "확인을 위해 사용자명을 입력하세요": "確認のためユーザー名を入力してください",
  "기본 정보": "基本情報",
  "생년월일을 확인하세요.": "生年月日を確認してください。",
  "국가": "国",
  "국가·성별": "国・性別",
  "이 정보는 다음 날짜 이후 다시 변경할 수 있습니다.": "この情報は次の日付以降に再変更できます:",
  "약관이 업데이트되었습니다": "規約が更新されました",
  "계속하려면 변경된 내용을 확인하고 동의해주세요.": "続けるには、変更内容をご確認のうえ同意してください。",
  "위 내용을 확인했으며 동의합니다": "上記の内容を確認し、同意します",
  "동의하고 계속": "同意して続ける",
  "동의 처리 중 문제가 발생했습니다. 다시 시도해주세요.": "同意の保存中に問題が発生しました。もう一度お試しください。",
  "비밀번호 표시": "パスワードを表示",
  "비밀번호 숨기기": "パスワードを隠す",
  "입력하신 이메일·문의 내용은 답변 목적으로만 사용되며, 처리 후 파기됩니다.": "ご入力のメールアドレスとお問い合わせ内容は返信のみに使用し、処理後に破棄します。",
  "문서 언어": "文書の言語",
  "메뉴": "メニュー",
  "탐색": "ナビゲーション",
  "이 사용자를 차단할까요?": "このユーザーをブロックしますか？",
  "알림을 모두 지울까요?": "通知をすべて消去しますか？",
  "만들지 못했어요. 잠시 후 다시 시도해주세요.": "作成できませんでした。しばらくしてからもう一度お試しください。",
  "아직 없습니다.": "まだありません。",
  평점순: "評価順",
  좋아요순: "いいね順",

  곡: "曲",
  앨범: "アルバム",
  리뷰: "レビュー",
  "{count}개 평가": "{count} 件の評価",
  "Spotify에서 듣기": "Spotify で聴く",
  "YouTube에서 보기": "YouTube で見る",
  트랙리스트: "トラックリスト",
  정보: "情報",
  "{count}곡": "{count} 曲",
  발매일: "リリース日",
  "트랙 수": "トラック数",
  저작권: "著作権",
  "평점 추이": "評価の推移",
  추이: "推移",
  "(수정됨)": "(編集済み)",
  "이 댓글을 삭제할까요?": "このコメントを削除しますか？",
  "평가가 더 쌓이면 추이가 표시됩니다.": "評価が増えると推移が表示されます。",
  "MusicBrainz에서 보기": "MusicBrainz で見る",
  장르: "ジャンル",
  "아직 이 장르의 리뷰가 없어요.": "このジャンルのレビューはまだありません。",
  "아직 장르 태그가 없습니다.": "まだジャンルタグがありません。",
  길이: "長さ",
  "아직 리뷰가 없습니다. 첫 평가를 남겨보세요.": "まだレビューがありません。最初の評価を残そう。",
  인기순: "人気順",
  최신순: "新着順",
  평가하기: "評価する",
  "내 평가 수정": "評価を編集",

  "검색어를 입력하세요.": "検索ワードを入力してください。",
  "검색 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.":
    "問題が発生しました。しばらくして再試行してください。",
  '"{q}"에 대한 검색 결과가 없습니다.': '"{q}" の検索結果がありません。',
  "이 카테고리엔 결과가 없습니다.": "このカテゴリには結果がありません。",
  "더 보기": "もっと見る",
  "검색 중…": "検索中…",
  아티스트: "アーティスト",
  유저: "ユーザー",

  "별점을 선택해주세요.": "評価を選んでください。",
  "리뷰를 남겨주세요 (최소 10자)": "レビューを書く（10文字以上）",
  "리뷰는 최소 10자 이상 작성해주세요.": "レビューは10文字以上で入力してください。",
  "최소 10자 이상 작성해주세요.": "10文字以上入力してください。",
  "리뷰를 작성하면 경험치 10 XP를 받아요.": "レビューを書くと経験値10 XPを獲得します。",
  "평가를 삭제하면 경험치 10 XP가 차감돼요. 삭제할까요?": "評価を削除すると獲得した経験値10 XPが差し引かれます。削除しますか？",
  "욕설·비방은 금지됩니다. 비판은 좋지만 비난은 안 됩니다.":
    "暴言・中傷は禁止です。批評は歓迎ですが、個人攻撃はいけません。",
  "부적절한 리뷰는 신고 없이도 관리자가 삭제하고 제재할 수 있습니다.":
    "不適切なレビューは通報がなくても管理者が削除・処分することがあります。",
  삭제: "削除",
  취소: "キャンセル",
  "저장 중…": "保存中…",
  수정: "更新",
  수정하기: "編集",
  등록: "投稿",

  "음악과 무관한 내용": "音楽と無関係な内容",
  "악플·욕설": "誹謗中傷・暴言",
  "부적절한 이름·프로필 사진": "不適切な名前・プロフィール画像",
  기타: "その他",
  "사유를 선택해주세요.": "理由を選んでください。",
  신고: "通報",
  "신고가 접수되었습니다.": "通報を受け付けました。",
  닫기: "閉じる",
  "리뷰 신고": "レビューを通報",
  "유저 신고": "ユーザーを通報",
  "신고 내용은 운영자가 검토합니다.": "通報内容は運営が確認します。",
  "상세 내용 (선택)": "詳細（任意）",
  "접수 중…": "送信中…",
  "관심 없음": "興味なし",
  "피드에서 숨겼어요": "フィードから非表示にしました",
  "실행 취소": "元に戻す",
  더보기: "もっと",
  차단: "ブロック",
  "차단 해제": "ブロック解除",
  "차단한 사용자입니다. 리뷰를 보려면 차단을 해제하세요.": "ブロック中のユーザーです。レビューを見るにはブロックを解除してください。",
  "차단 관리": "ブロック管理",
  "차단한 유저는 서로의 리뷰·프로필이 보이지 않습니다.": "ブロックしたユーザーとは互いのレビュー・プロフィールが見えません。",
  "차단한 유저가 없습니다.": "ブロックしたユーザーはいません。",

  "님이 회원님을 팔로우했습니다": "さんがあなたをフォローしました",
  "님이 회원님의 리뷰를 좋아합니다": "さんがあなたのレビューにいいねしました",
  "님이 댓글에서 회원님을 언급했습니다": "さんがコメントであなたをメンションしました",
  "알림이 없습니다.": "通知はありません。",
  "알림 설정": "通知設定",
  "모두 지우기": "すべて消去",
  "알림 삭제": "通知を削除",

  환영합니다: "ようこそ",
  이메일: "メール",
  비밀번호: "パスワード",
  로그인: "ログイン",
  "비밀번호를 잊으셨나요?": "パスワードをお忘れですか？",
  또는: "または",
  "처음이신가요?": "はじめてですか？",
  회원가입: "新規登録",
  로그인됨: "ログイン済み",

  "Google로 계속하기": "Google で続ける",
  "Discord로 계속하기": "Discord で続ける",

  "6자리 코드를 입력하세요.": "6桁のコードを入力してください。",
  "코드가 올바르지 않거나 만료되었습니다.": "コードが正しくないか、期限切れです。",
  "인증 메일을 보냈습니다": "確認メールを送信しました",
  "받은 편지함에서 6자리 코드를 입력하세요.": "受信トレイの6桁のコードを入力してください。",
  "6자리 코드": "6桁のコード",
  확인: "確認",
  "메일이 안 보이면 스팸함을 확인하세요.": "メールが見つからない場合は迷惑メールを確認してください。",
  "가입을 환영합니다": "アカウントを作成",
  "확인 중…": "確認中…",
  "사용 가능": "利用可能",
  "올바른 이메일 형식이 아닙니다.": "有効なメールアドレスではありません。",
  "이미 가입된 이메일입니다.": "このメールは既に登録されています。",
  "8자 이상": "8文字以上",
  "대소문자 포함": "大文字と小文字を含む",
  "숫자 포함": "数字を含む",
  "특수문자 포함": "記号を含む",
  "비밀번호 확인": "パスワード確認",
  "비밀번호가 일치하지 않습니다.": "パスワードが一致しません。",
  "{link}에 동의합니다.": "{link} に同意します。",
  가입하기: "登録する",
  "이미 계정이 있으신가요?": "すでにアカウントをお持ちですか？",

  "거의 다 됐어요": "あと少し",
  "프로필을 완성해주세요.": "プロフィールを完成させてください。",
  "영문·숫자·하이픈, 2–30자.": "英字・数字・ハイフン、2〜30文字。",
  "이미 사용 중": "使用中",
  "사용할 수 없는 형식": "使用できない形式",
  년: "年",
  월: "月",
  일: "日",
  "만 14세 이상만 가입할 수 있습니다.": "14歳以上のみ登録できます。",
  "올바른 날짜를 선택하세요.": "正しい日付を選んでください。",
  생년월일: "生年月日",
  "성별 (선택)": "性別（任意）",
  남성: "男性",
  여성: "女性",
  비공개: "非公開",
  완료: "完了",

  "재설정 링크를 보냈습니다": "再設定リンクを送信しました",
  "{email}의 메일함을 확인해주세요.": "{email} の受信トレイを確認してください。",
  로그인으로: "ログインへ",
  "비밀번호 재설정": "パスワード再設定",
  "가입한 이메일로 재설정 링크를 보내드립니다.": "登録済みのメールに再設定リンクを送ります。",
  "재설정 링크 보내기": "再設定リンクを送る",
  "로그인으로 돌아가기": "ログインに戻る",
  "비밀번호는 8자 이상이어야 합니다.": "パスワードは8文字以上にしてください。",
  "비밀번호가 변경되었습니다": "パスワードを変更しました",
  "새 비밀번호로 다시 로그인해주세요.": "新しいパスワードで再度ログインしてください。",
  로그인하기: "ログイン",
  "유효하지 않은 접근입니다": "無効なアクセスです",
  "재설정 링크가 만료되었거나 잘못되었습니다.": "再設定リンクが期限切れか無効です。",
  "재설정 링크 다시 받기": "再設定リンクを再取得",
  "새 비밀번호 설정": "新しいパスワードを設定",
  "새 비밀번호 (8자 이상)": "新しいパスワード（8文字以上）",
  "새 비밀번호 확인": "新しいパスワード確認",
  "비밀번호 변경": "パスワード変更",

  "변경되었습니다.": "更新しました。",
  "username 형식(2~30자, 영문/숫자/하이픈)을 확인하세요.":
    "ユーザー名の形式（2〜30文字、英数字・ハイフン）を確認してください。",
  "설정되었습니다.": "設定しました。",
  "계정 정보": "アカウント情報",
  가입일: "登録日",
  "연동 계정": "連携アカウント",
  아바타: "アバター",
  "업로드 중…": "アップロード中…",
  "이미지 변경": "画像を変更",
  "jpg, png, webp · 최대 5MB": "jpg, png, webp · 最大5MB",
  닉네임: "ニックネーム",
  변경: "保存",
  "비밀번호 설정": "パスワード設定",
  "소셜 로그인 계정입니다. 비밀번호를 설정하면 이메일로도 로그인할 수 있습니다.":
    "ソーシャルログインのアカウントです。パスワードを設定するとメールでもログインできます。",
  "새 비밀번호": "新しいパスワード",
  "회원 탈퇴": "退会",
  "계정과 모든 데이터가 영구 삭제됩니다.": "アカウントとすべてのデータが完全に削除されます。",
  "처리 중…": "処理中…",
  "이메일 변경": "メール変更",
  "새 이메일로 확인 메일을 보냈습니다. 링크를 눌러야 변경이 완료됩니다.":
    "新しいメールに確認メールを送信しました。リンクを押すと変更が完了します。",
  "변경하려면 새 이메일로 온 확인 링크를 눌러야 합니다.":
    "変更するには、新しいメールに届いた確認リンクを押してください。",
  "공개 설정": "公開設定",
  "리뷰어 레벨 숨기기": "レビュアーレベルを非表示",
  "켜면 피드·리뷰·차트·프로필에서 내 레벨 뱃지가 표시되지 않습니다.": "オンにすると、フィード・レビュー・チャート・プロフィールでレベルバッジが表示されません。",
  "데이터 내보내기": "データのエクスポート",
  "내 프로필·평가·팔로우·댓글을 JSON 파일로 내려받습니다.":
    "プロフィール・評価・フォロー・コメントを JSON ファイルでダウンロードします。",
  "JSON 내보내기": "JSON をエクスポート",
  "내보내는 중…": "エクスポート中…",
  "내보내기 파일을 저장했습니다.": "エクスポートファイルを保存しました。",

  "전체 알림": "すべての通知",
  "모든 알림을 받습니다. 끄면 아래 항목과 무관하게 알림이 오지 않습니다.":
    "すべての通知を受け取ります。オフにすると、下の項目に関係なく通知は届きません。",
  "팔로우 알림": "フォロー通知",
  "다른 유저가 회원님을 팔로우할 때": "他のユーザーがあなたをフォローしたとき",
  "리뷰 좋아요 알림": "レビューいいね通知",
  "회원님의 리뷰에 좋아요가 달릴 때": "あなたのレビューにいいねが付いたとき",

  "준비 중입니다.": "近日公開。",

  "{score}점": "{score} 点",

  댓글: "コメント",
  "댓글 {count}": "コメント {count} 件",
  "첫 댓글을 남겨보세요.": "最初のコメントを残そう。",
  "댓글 달기…": "コメントを追加…",
  "로그인하고 댓글 달기": "ログインしてコメント",
  답글: "返信",
  "답글 {count}개 보기": "返信{count}件を表示",
  간략히: "折りたたむ",
  "평가 없음": "評価なし",
  "삭제된 댓글입니다.": "削除されたコメントです。",
  "댓글 신고": "コメントを通報",
  "멘션 알림": "メンション通知",
  "다른 유저가 댓글에서 회원님을 @태그할 때": "他のユーザーがコメントであなたを@メンションしたとき",
  "멘션 알림 끄기": "メンションをミュート",
  "멘션 알림 꺼짐": "メンションをミュート中",
  "이 페이지에서 멘션 알림 끄기": "このページのメンション通知をオフにする",
  "이 페이지의 멘션 알림이 꺼져 있습니다": "このページのメンション通知はオフです",
  믹스: "ミックス",
  "믹스 만들기": "ミックスを作る",
  "믹스 제목": "ミックスのタイトル",
  "한마디 (선택) — 이거 들어봐": "ひとこと（任意）— 聴いてみて",
  "듣기 링크 (선택) — 유튜브·스포티파이 등": "再生リンク（任意）— YouTube・Spotifyなど",
  만들기: "作成",
  "만드는 중…": "作成中…",
  듣기: "再生",
  "이 믹스를 삭제할까요?": "このミックスを削除しますか？",
  "아직 담긴 곡이 없습니다.": "まだ曲がありません。",
  "곡 검색해서 담기…": "曲を検索して追加…",
  "아직 믹스가 없습니다.": "まだミックスがありません。",
  담기: "追加",
  "최대 {count}곡까지 담을 수 있어요.": "最大{count}曲まで。",
  "유튜브 링크": "YouTubeリンク",
  "유튜브에서 보기": "YouTubeで見る",
  "http(s) 링크만 넣을 수 있어요.": "http(s) のリンクのみ入力できます。",
  설명: "説明",
  "플레이리스트 링크": "プレイリストリンク",
  "위로 이동": "上へ",
  저장: "保存",
  "곡 수정": "曲を編集",
  "노래 변경": "曲を変更",
  "믹스 검색": "ミックスを検索",
  "믹스 수정": "ミックスを編集",
  목록으로: "一覧へ",
  발견으로: "発見へ",
  "믹스 신고": "ミックスを通報",
  안내: "ヘルプ",
  저장됨: "保存しました",
  "순서는 화살표로 바꿀 수 있어요.": "矢印で並べ替えできます。",
  "곡을 검색해 담고, 유튜브 링크를 붙이면 스포티파이 없이도 들을 수 있어요.": "曲を検索して追加し、YouTubeリンクを付ければSpotifyがなくても聴けます。",
  "아래로 이동": "下へ",
  공유: "共有",
  복사됨: "コピーしました",
  "{username}님의 리뷰": "{username} さんのレビュー",

  "Spotify에서 열기": "Spotify で開く",
  디스코그래피: "ディスコグラフィ",
  싱글: "シングル",
  컴필레이션: "コンピレーション",
  "평가 좋은 트랙": "高評価の曲",
  "평가 좋은 앨범": "高評価のアルバム",
  "아직 평가된 곡이 없습니다.": "まだ評価された曲がありません。",
  "아직 평가된 앨범이 없습니다.": "まだ評価されたアルバムがありません。",
  "평가된 릴리스 {rated} · 총 평가 {total}": "評価されたリリース {rated} · 総評価 {total}",
  "커뮤니티 최근 리뷰": "コミュニティの最新レビュー",
  "음악 정보를 불러오지 못했습니다. 잠시 후 다시 시도해주세요.":
    "音楽情報を読み込めませんでした。しばらくして再試行してください。",
  "화제의 릴리스": "話題のリリース",
};

// 한국어 → 스페인어.
const ES: Record<string, string> = {
  "음악을 듣고 평가하고 기록하세요.": "Escucha, califica y guarda la música que amas.",

  신규: "Nuevo",
  급상승: "En alza",
  차트: "Rankings",
  발견: "Descubrir",
  추천: "Recomendado",
  "당신이 좋아하는 장르들": "Géneros que te gustan",
  "오늘의 음악": "Selección del día",
  "리뷰하기": "Escribir reseña",
  "선호 장르": "Géneros favoritos",
  "내 선호 장르": "Mis géneros",
  "추천에 반영돼요. 언제든 바꿀 수 있어요.": "Se usa para recomendaciones. Cámbialo cuando quieras.",
  "저장됐어요": "Guardado",
  "좋아하는 장르 (선택) — 추천에 반영돼요.": "Géneros que te gustan (opcional) — se usan para recomendaciones.",
  "로그인 / 가입": "Entrar / Registrarse",
  홈: "Inicio",
  "앨범, 곡, 아티스트, 유저 검색": "Buscar álbumes, canciones, artistas, usuarios",

  프로필: "Perfil",
  업적: "Logros",
  설정: "Ajustes",
  로그아웃: "Cerrar sesión",
  "로그아웃 하시겠어요?": "¿Cerrar sesión?",

  정책: "Políticas",
  고객지원: "Soporte",
  소식: "Novedades",
  "서비스 소개": "Acerca de",
  후원하기: "Apóyanos",
  "Glitter는 한 사람이 운영하는 서비스입니다. 서버비(호스팅·스토리지)에 커피 한 잔이 큰 힘이 됩니다.": "Glitter lo lleva una sola persona. Un café para los costes de servidor (hosting y almacenamiento) ayuda mucho.",
  "순수 후원이라 어떤 대가나 혜택도 없어요. 부담 갖지 마시고, 마음이 있으실 때만.": "Es una donación pura, sin recompensas ni ventajas. Sin presión, solo si te apetece.",
  "Ko-fi에서 후원하기": "Apoyar en Ko-fi",
  "후원 창구는 곧 열립니다.": "Las donaciones se abrirán pronto.",
  "결제·환불은 외부 후원 플랫폼에서 안전하게 처리됩니다.": "Los pagos y reembolsos los gestiona de forma segura una plataforma externa.",
  공지사항: "Anuncios",
  패치노트: "Notas de versión",
  "페이지 이동": "Paginación",
  릴리스: "Lanzamientos",
  "작업 중": "En progreso",
  "등록된 패치노트가 없습니다.": "Aún no hay notas de versión.",
  "등록된 공지가 없습니다.": "Aún no hay anuncios.",
  운영자: "Operador",
  "조회 {count}": "{count} vista|{count} vistas",
  이전: "Anterior",
  다음: "Siguiente",
  "별점을 매길수록 별이 쌓여 반짝입니다 — 그래서 Glitter.":
    "Cuanto más calificas, más estrellas se acumulan y brillan — por eso, Glitter.",
  이용약관: "Términos del servicio",
  "개인정보 처리방침": "Política de privacidad",
  문의: "Contacto",

  "음악을 듣고, 평가하고, 기록하세요.": "Escucha. Califica. Recuerda.",
  "좋아하는 앨범과 곡을 평가하고, 리뷰를 남기고, 취향이 맞는 사람을 팔로우하세요.":
    "Califica tus álbumes y canciones favoritos, escribe reseñas y sigue a quienes comparten tu gusto.",
  둘러보기: "Explorar",
  시작하기: "Empezar",
  "0.5점 단위 별점": "Calificaciones en pasos de media estrella",
  "곡·앨범을 세밀하게 평가하고 나만의 기록을 남기세요.":
    "Califica canciones y álbumes con precisión y lleva tu propio registro.",
  "취향 팔로우": "Sigue gustos",
  "취향이 맞는 유저를 팔로우하고 새 평가를 받아보세요.":
    "Sigue a usuarios con tu gusto y entérate de sus nuevas calificaciones.",
  "곡을 골라 '이거 들어봐' 믹스를 만들면, 유튜브·스포티파이로 듣고 평가해요.":
    "Elige pistas para un mix de 'escucha esto' — otros lo oyen en YouTube o Spotify y lo califican.",
  "리뷰 & 댓글": "Reseñas y comentarios",
  "리뷰를 쓰고 댓글로 이야기 나누세요. 좋은 리뷰엔 좋아요.": "Escribe reseñas y comenta. Dale me gusta a las buenas.",
  "발견 & 차트": "Descubre y rankings",
  "신규·급상승 음악을 발견하고, 랭킹으로 흐름을 봐요.": "Encuentra música nueva y en auge, y sigue los rankings.",
  "곡·앨범 평점이 시간에 따라 어떻게 움직이는지 그래프로 봐요.": "Mira cómo cambia la calificación de una pista o álbum con el tiempo.",

  계정: "Cuenta",
  알림: "Notificaciones",
  화면: "Pantalla",
  연동: "Integraciones",
  언어: "Idioma",
  테마: "Tema",
  라이트: "Claro",
  다크: "Oscuro",
  시스템: "Sistema",
  "화면 밝기 테마를 선택합니다. 시스템은 기기 설정을 따릅니다.":
    "Elige el tema de la pantalla. El sistema sigue la configuración del dispositivo.",
  "표시 언어입니다.": "El idioma que se muestra en la app.",

  팔로워: "Seguidores",
  팔로잉: "Siguiendo",
  팔로우: "Seguir",
  "프로필 편집": "Editar perfil",
  "작성한 리뷰": "Reseñas",
  "받은 좋아요 {count}": "{count} me gusta recibido|{count} me gusta recibidos",
  "리뷰 {count}": "{count} reseña|{count} reseñas",
  "다음 레벨까지 {xp} XP": "{xp} XP para el siguiente nivel",
  "아직 작성한 리뷰가 없습니다.": "Aún no hay reseñas.",
  "(알 수 없는 항목)": "(Elemento desconocido)",
  "관리자에 의해 삭제되었습니다.": "Eliminado por un administrador.",
  "계정이 영구 정지되어 작성이 제한됩니다.": "Tu cuenta está suspendida permanentemente; no puedes publicar.",
  "{until}까지 이용이 정지되어 작성이 제한됩니다.": "Estás suspendido hasta {until}; no puedes publicar.",
  "사유": "Motivo",
  "경고를 받았습니다.": "Has recibido una advertencia.",
  "새 공지사항이 등록되었어요.": "Se publicó un nuevo anuncio.",
  "일시 정지되어 평가할 수 없습니다.": "Estás suspendido y no puedes calificar.",
  "계정이 정지되어 평가할 수 없습니다.": "Tu cuenta está suspendida y no puedes calificar.",
  "시행일": "Fecha de entrada en vigor",
  "문서를 준비 중입니다.": "Este documento está en preparación.",
  "자주 묻는 질문": "Preguntas frecuentes",
  검색: "Buscar",
  "최근 검색": "Búsquedas recientes",
  "검색 결과가 없습니다.": "No se encontraron resultados.",
  "등록된 질문이 없습니다.": "Aún no hay preguntas.",
  "들은 음악을, 나만의 기록으로.": "Convierte lo que escuchas en tu propio registro.",
  "Glitter는 음악 감상을 기록하고 나누는 서비스입니다. 방대한 음악 카탈로그 위에서, 당신의 별점과 리뷰가 쌓여 취향의 지도가 됩니다.":
    "Glitter es un lugar para registrar y compartir lo que escuchas. Sobre un enorme catálogo musical, tus calificaciones y reseñas construyen un mapa de tu gusto.",
  "무엇을 할 수 있나요": "Qué puedes hacer",
  "Glitter에서 평가하고 리뷰하세요.": "Califica y reseña en Glitter.",
  "페이지를 찾을 수 없습니다.": "Página no encontrada.",
  "홈으로": "Ir al inicio",
  "다시 시도": "Reintentar",
  "일시적인 오류가 발생했습니다.": "Algo salió mal.",
  주: "Semana",
  최근: "Reciente",
  "내 최근 리뷰": "Mis reseñas recientes",
  전체: "Todo",
  "최다 리뷰": "Más reseñados",
  "최고 평가": "Mejor valorados",
  대상: "Tipo",
  정렬: "Orden",
  좋아요: "Me gusta",
  성별: "Género",
  나이대: "Edad",
  "10대": "10-19",
  "20대": "20-29",
  "30대": "30-39",
  "40대": "40-49",
  "50대+": "50+",
  "아직 차트에 오른 항목이 없습니다.": "Aún no hay nada en el ranking.",
  "음악 차트": "Ranking de música",
  "유저 차트": "Ranking de usuarios",
  "곡·앨범·아티스트 랭킹": "Canciones · Álbumes · Artistas",
  "리뷰어 랭킹": "Mejores reseñadores",
  화제순: "Popular",
  "좋아요 많은 순": "Más me gusta",
  "좋아요 비율 높은 순": "Mejor proporción",
  보기: "Ver",
  카드형: "Tarjeta",
  축약형: "Compacto",
  "정렬 기준": "Ordenar por",
  "피드가 비어 있습니다.": "Tu feed está vacío.",
  피드: "Feed",
  "이름의 의미": "Sobre el nombre",
  "지금 첫 별점을 남겨보세요.": "Deja tu primera calificación.",
  "문의하기": "Contáctanos",
  카테고리: "Categoría",
  제목: "Asunto",
  내용: "Mensaje",
  "평가·리뷰": "Calificaciones y reseñas",
  버그: "Error",
  보내기: "Enviar",
  "보내는 중…": "Enviando…",
  "문의가 접수되었습니다.": "Hemos recibido tu consulta.",
  "답변은 입력하신 이메일로 보내드립니다.": "Responderemos al correo que indicaste.",
  "이메일 형식을 확인하세요.": "Revisa el formato del correo.",
  "제목을 입력하세요.": "Introduce un asunto.",
  "내용을 입력하세요.": "Escribe tu mensaje.",
  "불러오는 중…": "Cargando…",
  "모두 확인했어요": "Estás al día",
  "본문으로 건너뛰기": "Saltar al contenido",
  "확인을 위해 사용자명을 입력하세요": "Escribe tu nombre de usuario para confirmar",
  "기본 정보": "Información básica",
  "생년월일을 확인하세요.": "Verifica tu fecha de nacimiento.",
  "국가": "País",
  "국가·성별": "País y género",
  "이 정보는 다음 날짜 이후 다시 변경할 수 있습니다.": "Podrás volver a cambiarlo después de",
  "약관이 업데이트되었습니다": "Hemos actualizado nuestros términos",
  "계속하려면 변경된 내용을 확인하고 동의해주세요.": "Revisa los cambios y acéptalos para continuar.",
  "위 내용을 확인했으며 동의합니다": "He leído y acepto lo anterior",
  "동의하고 계속": "Aceptar y continuar",
  "동의 처리 중 문제가 발생했습니다. 다시 시도해주세요.": "Ocurrió un problema al guardar tu consentimiento. Inténtalo de nuevo.",
  "비밀번호 표시": "Mostrar contraseña",
  "비밀번호 숨기기": "Ocultar contraseña",
  "입력하신 이메일·문의 내용은 답변 목적으로만 사용되며, 처리 후 파기됩니다.": "Tu correo y mensaje se usan solo para responder y se eliminan después.",
  "문서 언어": "Idioma del documento",
  "메뉴": "Menú",
  "탐색": "Navegación",
  "이 사용자를 차단할까요?": "¿Bloquear a este usuario?",
  "알림을 모두 지울까요?": "¿Borrar todas las notificaciones?",
  "만들지 못했어요. 잠시 후 다시 시도해주세요.": "No se pudo crear. Inténtalo de nuevo en un momento.",
  "아직 없습니다.": "Nada todavía.",
  평점순: "Por calificación",
  좋아요순: "Más gustados",

  곡: "Canción",
  앨범: "Álbum",
  리뷰: "Reseñas",
  "{count}개 평가": "{count} calificación|{count} calificaciones",
  "Spotify에서 듣기": "Escuchar en Spotify",
  "YouTube에서 보기": "Ver en YouTube",
  트랙리스트: "Lista de canciones",
  정보: "Información",
  "{count}곡": "{count} canción|{count} canciones",
  발매일: "Fecha de lanzamiento",
  "트랙 수": "Canciones",
  저작권: "Derechos de autor",
  "평점 추이": "Tendencia de calificación",
  추이: "Tendencia",
  "(수정됨)": "(editado)",
  "이 댓글을 삭제할까요?": "¿Eliminar este comentario?",
  "평가가 더 쌓이면 추이가 표시됩니다.": "La tendencia aparece cuando se acumulan más calificaciones.",
  "MusicBrainz에서 보기": "Ver en MusicBrainz",
  장르: "Género",
  "아직 이 장르의 리뷰가 없어요.": "Aún no hay reseñas en este género.",
  "아직 장르 태그가 없습니다.": "Aún no hay etiquetas de género.",
  길이: "Duración",
  "아직 리뷰가 없습니다. 첫 평가를 남겨보세요.": "Aún no hay reseñas. Sé el primero en calificar.",
  인기순: "Popular",
  최신순: "Recientes",
  평가하기: "Calificar",
  "내 평가 수정": "Editar mi calificación",

  "검색어를 입력하세요.": "Introduce un término de búsqueda.",
  "검색 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.":
    "Algo salió mal. Inténtalo de nuevo en un momento.",
  '"{q}"에 대한 검색 결과가 없습니다.': 'No hay resultados para "{q}".',
  "이 카테고리엔 결과가 없습니다.": "No hay resultados en esta categoría.",
  "더 보기": "Ver más",
  "검색 중…": "Buscando…",
  아티스트: "Artista",
  유저: "Usuario",

  "별점을 선택해주세요.": "Selecciona una calificación.",
  "리뷰를 남겨주세요 (최소 10자)": "Escribe una reseña (mín. 10 caracteres)",
  "리뷰는 최소 10자 이상 작성해주세요.": "Las reseñas deben tener al menos 10 caracteres.",
  "최소 10자 이상 작성해주세요.": "Escribe al menos 10 caracteres.",
  "리뷰를 작성하면 경험치 10 XP를 받아요.": "Ganas 10 XP por escribir una reseña.",
  "평가를 삭제하면 경험치 10 XP가 차감돼요. 삭제할까요?": "Al eliminar la calificación se descuentan los 10 XP obtenidos. ¿Eliminar?",
  "욕설·비방은 금지됩니다. 비판은 좋지만 비난은 안 됩니다.":
    "Nada de insultos ni difamación. La crítica es bienvenida; los ataques personales, no.",
  "부적절한 리뷰는 신고 없이도 관리자가 삭제하고 제재할 수 있습니다.":
    "Las reseñas inapropiadas pueden ser eliminadas y sancionadas por un administrador sin necesidad de reporte.",
  삭제: "Eliminar",
  취소: "Cancelar",
  "저장 중…": "Guardando…",
  수정: "Actualizar",
  수정하기: "Editar",
  등록: "Publicar",

  "음악과 무관한 내용": "No trata de música",
  "악플·욕설": "Insultos o abuso",
  "부적절한 이름·프로필 사진": "Nombre o foto de perfil inapropiados",
  기타: "Otro",
  "사유를 선택해주세요.": "Selecciona un motivo.",
  신고: "Reportar",
  "신고가 접수되었습니다.": "Tu reporte ha sido enviado.",
  닫기: "Cerrar",
  "리뷰 신고": "Reportar reseña",
  "유저 신고": "Reportar usuario",
  "신고 내용은 운영자가 검토합니다.": "Nuestros moderadores revisan los reportes.",
  "상세 내용 (선택)": "Detalles (opcional)",
  "접수 중…": "Enviando…",
  "관심 없음": "No me interesa",
  "피드에서 숨겼어요": "Ocultado de tu feed",
  "실행 취소": "Deshacer",
  더보기: "Más",
  차단: "Bloquear",
  "차단 해제": "Desbloquear",
  "차단한 사용자입니다. 리뷰를 보려면 차단을 해제하세요.": "Has bloqueado a este usuario. Desbloquéalo para ver sus reseñas.",
  "차단 관리": "Usuarios bloqueados",
  "차단한 유저는 서로의 리뷰·프로필이 보이지 않습니다.": "Tú y los usuarios bloqueados no pueden ver sus reseñas ni perfiles.",
  "차단한 유저가 없습니다.": "No has bloqueado a nadie.",

  "님이 회원님을 팔로우했습니다": " te siguió",
  "님이 회원님의 리뷰를 좋아합니다": " le dio me gusta a tu reseña",
  "님이 댓글에서 회원님을 언급했습니다": " te mencionó en un comentario",
  "알림이 없습니다.": "No hay notificaciones.",
  "알림 설정": "Ajustes de notificaciones",
  "모두 지우기": "Borrar todo",
  "알림 삭제": "Eliminar notificación",

  환영합니다: "Bienvenido",
  이메일: "Correo",
  비밀번호: "Contraseña",
  로그인: "Entrar",
  "비밀번호를 잊으셨나요?": "¿Olvidaste tu contraseña?",
  또는: "o",
  "처음이신가요?": "¿Eres nuevo?",
  회원가입: "Registrarse",
  로그인됨: "Sesión iniciada",

  "Google로 계속하기": "Continuar con Google",
  "Discord로 계속하기": "Continuar con Discord",

  "6자리 코드를 입력하세요.": "Introduce el código de 6 dígitos.",
  "코드가 올바르지 않거나 만료되었습니다.": "El código es incorrecto o ha expirado.",
  "인증 메일을 보냈습니다": "Enviamos un correo de verificación",
  "받은 편지함에서 6자리 코드를 입력하세요.": "Introduce el código de 6 dígitos de tu bandeja de entrada.",
  "6자리 코드": "Código de 6 dígitos",
  확인: "Confirmar",
  "메일이 안 보이면 스팸함을 확인하세요.": "Si no lo ves, revisa tu carpeta de spam.",
  "가입을 환영합니다": "Crea tu cuenta",
  "확인 중…": "Comprobando…",
  "사용 가능": "Disponible",
  "올바른 이메일 형식이 아닙니다.": "No es una dirección de correo válida.",
  "이미 가입된 이메일입니다.": "Este correo ya está registrado.",
  "8자 이상": "Al menos 8 caracteres",
  "대소문자 포함": "Mayúsculas y minúsculas",
  "숫자 포함": "Un número",
  "특수문자 포함": "Un carácter especial",
  "비밀번호 확인": "Confirmar contraseña",
  "비밀번호가 일치하지 않습니다.": "Las contraseñas no coinciden.",
  "{link}에 동의합니다.": "Acepto los {link}.",
  가입하기: "Registrarse",
  "이미 계정이 있으신가요?": "¿Ya tienes una cuenta?",

  "거의 다 됐어요": "Casi listo",
  "프로필을 완성해주세요.": "Completa tu perfil.",
  "영문·숫자·하이픈, 2–30자.": "Letras, números, guiones. 2–30 caracteres.",
  "이미 사용 중": "Ya en uso",
  "사용할 수 없는 형식": "Formato no válido",
  년: "Año",
  월: "Mes",
  일: "Día",
  "만 14세 이상만 가입할 수 있습니다.": "Debes tener al menos 14 años.",
  "올바른 날짜를 선택하세요.": "Selecciona una fecha válida.",
  생년월일: "Fecha de nacimiento",
  "성별 (선택)": "Género (opcional)",
  남성: "Hombre",
  여성: "Mujer",
  비공개: "Sin especificar",
  완료: "Listo",

  "재설정 링크를 보냈습니다": "Enlace de restablecimiento enviado",
  "{email}의 메일함을 확인해주세요.": "Revisa la bandeja de entrada de {email}.",
  로그인으로: "A iniciar sesión",
  "비밀번호 재설정": "Restablecer contraseña",
  "가입한 이메일로 재설정 링크를 보내드립니다.": "Enviaremos un enlace de restablecimiento a tu correo.",
  "재설정 링크 보내기": "Enviar enlace",
  "로그인으로 돌아가기": "Volver a iniciar sesión",
  "비밀번호는 8자 이상이어야 합니다.": "La contraseña debe tener al menos 8 caracteres.",
  "비밀번호가 변경되었습니다": "Contraseña cambiada",
  "새 비밀번호로 다시 로그인해주세요.": "Vuelve a iniciar sesión con tu nueva contraseña.",
  로그인하기: "Entrar",
  "유효하지 않은 접근입니다": "Acceso no válido",
  "재설정 링크가 만료되었거나 잘못되었습니다.": "El enlace ha expirado o no es válido.",
  "재설정 링크 다시 받기": "Obtener un nuevo enlace",
  "새 비밀번호 설정": "Establecer nueva contraseña",
  "새 비밀번호 (8자 이상)": "Nueva contraseña (8+ caracteres)",
  "새 비밀번호 확인": "Confirmar nueva contraseña",
  "비밀번호 변경": "Cambiar contraseña",

  "변경되었습니다.": "Actualizado.",
  "username 형식(2~30자, 영문/숫자/하이픈)을 확인하세요.":
    "Revisa el formato del nombre de usuario (2–30 caracteres, letras/números/guiones).",
  "설정되었습니다.": "Establecido.",
  "계정 정보": "Información de la cuenta",
  가입일: "Registro",
  "연동 계정": "Cuentas vinculadas",
  아바타: "Avatar",
  "업로드 중…": "Subiendo…",
  "이미지 변경": "Cambiar imagen",
  "jpg, png, webp · 최대 5MB": "jpg, png, webp · hasta 5MB",
  닉네임: "Apodo",
  변경: "Guardar",
  "비밀번호 설정": "Establecer contraseña",
  "소셜 로그인 계정입니다. 비밀번호를 설정하면 이메일로도 로그인할 수 있습니다.":
    "Esta es una cuenta de inicio de sesión social. Establece una contraseña para entrar también con correo.",
  "새 비밀번호": "Nueva contraseña",
  "회원 탈퇴": "Eliminar cuenta",
  "계정과 모든 데이터가 영구 삭제됩니다.": "Tu cuenta y todos tus datos se eliminarán permanentemente.",
  "처리 중…": "Procesando…",
  "이메일 변경": "Cambiar correo",
  "새 이메일로 확인 메일을 보냈습니다. 링크를 눌러야 변경이 완료됩니다.":
    "Se envió un correo de confirmación a la nueva dirección. Haz clic en el enlace para completar el cambio.",
  "변경하려면 새 이메일로 온 확인 링크를 눌러야 합니다.":
    "Para cambiarlo, haz clic en el enlace de confirmación enviado a tu nuevo correo.",
  "공개 설정": "Privacidad",
  "리뷰어 레벨 숨기기": "Ocultar nivel de reseñador",
  "켜면 피드·리뷰·차트·프로필에서 내 레벨 뱃지가 표시되지 않습니다.": "Si se activa, tu insignia de nivel se oculta en el feed, las reseñas, las listas y el perfil.",
  "데이터 내보내기": "Exportar datos",
  "내 프로필·평가·팔로우·댓글을 JSON 파일로 내려받습니다.":
    "Descarga tu perfil, calificaciones, seguidos y comentarios en un archivo JSON.",
  "JSON 내보내기": "Exportar JSON",
  "내보내는 중…": "Exportando…",
  "내보내기 파일을 저장했습니다.": "Se ha guardado tu archivo de exportación.",

  "전체 알림": "Todas las notificaciones",
  "모든 알림을 받습니다. 끄면 아래 항목과 무관하게 알림이 오지 않습니다.":
    "Recibe todas las notificaciones. Si lo desactivas, no se envía nada, sin importar las opciones de abajo.",
  "팔로우 알림": "Notificaciones de seguidores",
  "다른 유저가 회원님을 팔로우할 때": "Cuando otro usuario te sigue",
  "리뷰 좋아요 알림": "Notificaciones de me gusta",
  "회원님의 리뷰에 좋아요가 달릴 때": "Cuando a alguien le gusta tu reseña",

  "준비 중입니다.": "Próximamente.",

  "{score}점": "{score} estrella|{score} estrellas",

  댓글: "Comentarios",
  "댓글 {count}": "{count} comentario|{count} comentarios",
  "첫 댓글을 남겨보세요.": "Sé el primero en comentar.",
  "댓글 달기…": "Añadir un comentario…",
  "로그인하고 댓글 달기": "Inicia sesión para comentar",
  답글: "Responder",
  "답글 {count}개 보기": "Ver {count} respuesta|Ver {count} respuestas",
  간략히: "Mostrar menos",
  "평가 없음": "Sin calificación",
  "삭제된 댓글입니다.": "Este comentario fue eliminado.",
  "댓글 신고": "Reportar comentario",
  "멘션 알림": "Notificaciones de menciones",
  "다른 유저가 댓글에서 회원님을 @태그할 때": "Cuando alguien te menciona con @ en un comentario",
  "멘션 알림 끄기": "Silenciar menciones",
  "멘션 알림 꺼짐": "Menciones silenciadas",
  "이 페이지에서 멘션 알림 끄기": "Silenciar menciones en esta página",
  "이 페이지의 멘션 알림이 꺼져 있습니다": "Las menciones están silenciadas en esta página",
  믹스: "Mix",
  "믹스 만들기": "Crear un mix",
  "믹스 제목": "Título del mix",
  "한마디 (선택) — 이거 들어봐": "Una nota (opcional) — escucha esto",
  "듣기 링크 (선택) — 유튜브·스포티파이 등": "Enlace para escuchar (opcional) — YouTube, Spotify, etc.",
  만들기: "Crear",
  "만드는 중…": "Creando…",
  듣기: "Escuchar",
  "이 믹스를 삭제할까요?": "¿Eliminar este mix?",
  "아직 담긴 곡이 없습니다.": "Aún no hay pistas.",
  "곡 검색해서 담기…": "Buscar pistas para añadir…",
  "아직 믹스가 없습니다.": "Aún no hay mixes.",
  담기: "Añadir",
  "최대 {count}곡까지 담을 수 있어요.": "Hasta {count} pistas.",
  "유튜브 링크": "Enlace de YouTube",
  "유튜브에서 보기": "Ver en YouTube",
  "http(s) 링크만 넣을 수 있어요.": "Solo se permiten enlaces http(s).",
  설명: "Descripción",
  "플레이리스트 링크": "Enlace de playlist",
  "위로 이동": "Subir",
  저장: "Guardar",
  "곡 수정": "Editar pista",
  "노래 변경": "Cambiar canción",
  "믹스 검색": "Buscar mixes",
  "믹스 수정": "Editar mix",
  목록으로: "Volver a la lista",
  발견으로: "Volver a Descubrir",
  "믹스 신고": "Reportar mix",
  안내: "Ayuda",
  저장됨: "Guardado",
  "순서는 화살표로 바꿀 수 있어요.": "Reordena con las flechas.",
  "곡을 검색해 담고, 유튜브 링크를 붙이면 스포티파이 없이도 들을 수 있어요.": "Busca y añade pistas; agrega un enlace de YouTube para quienes no tengan Spotify.",
  "아래로 이동": "Bajar",
  공유: "Compartir",
  복사됨: "Copiado",
  "{username}님의 리뷰": "Reseña de {username}",

  "Spotify에서 열기": "Abrir en Spotify",
  디스코그래피: "Discografía",
  싱글: "Sencillo",
  컴필레이션: "Recopilación",
  "평가 좋은 트랙": "Canciones mejor valoradas",
  "평가 좋은 앨범": "Álbumes mejor valorados",
  "아직 평가된 곡이 없습니다.": "Aún no hay canciones calificadas.",
  "아직 평가된 앨범이 없습니다.": "Aún no hay álbumes calificados.",
  "평가된 릴리스 {rated} · 총 평가 {total}": "{rated} lanzamientos calificados · {total} calificaciones",
  "커뮤니티 최근 리뷰": "Reseñas recientes de la comunidad",
  "음악 정보를 불러오지 못했습니다. 잠시 후 다시 시도해주세요.":
    "No se pudo cargar la información musical. Inténtalo de nuevo en un momento.",
  "화제의 릴리스": "Lanzamientos en tendencia",
};

const DICTS: Record<Exclude<Locale, "ko">, Record<string, string>> = { en: EN, ja: JA, es: ES };
